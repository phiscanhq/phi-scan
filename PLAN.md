...
Created: March 15, 2026 | Updated: March 31, 2026 | Python 3.12.3 | uv 0.10.9
...
- [x] **4B.1** `phi_scan/compliance.py` — compliance framework mapping engine
- [x] **4B.2** Framework mappings (each finding tagged with applicable controls):
...
- [x] **4B.3** `--framework hipaa,soc2,hitrust,nist,gdpr,42cfr2,gina,cmia,bipa` flag —
  annotate findings with selected frameworks; `hipaa` always on; others opt-in
- [x] **4B.4** Report includes compliance matrix: which frameworks are violated by each finding
- [x] **4B.5** PDF/HTML reports include framework-specific sections when `--framework` is used
- [x] **4B.6** `phi-scan explain frameworks` — new explain topic listing all supported frameworks
  with full regulatory citation, enforcement body, and penalty ranges
- [x] **4B.7** `phi-scan explain deidentification` — explain HIPAA Safe Harbor vs Expert
...
- [x] **4C.1** `docs/confidence-scoring.md` — what confidence means, threshold ranges, per-layer breakdown, how to tune
- [x] **4C.2** `docs/hipaa-identifiers.md` — all 18 HIPAA PHI identifiers with examples and regex patterns
- [x] **4C.3** `docs/detection-layers.md` — how regex, NLP, FHIR, and AI layers work together
- [x] **4C.4** `docs/output-formats.md` — table, json, csv, sarif, pdf, html — when to use each, examples
- [x] **4C.5** `docs/remediation-guide.md` — per-category remediation playbook, synthetic data recommendations
- [x] **4C.6** `docs/changelog.md` — version history with breaking changes noted
- [x] **4C.7** `CONTRIBUTING.md` — how to contribute, code standards, PR process
- [x] **4C.8** Update README.md — add terminal screenshots, enterprise report examples
- [x] **4C.9** `docs/plugin-developer-guide.md` — how to build custom recognizers, register via entry points, test, and publish
- [x] **4C.10** `docs/compliance-frameworks.md` — complete regulatory reference:
...
- [x] **4C.11** `docs/de-identification.md` — HIPAA de-identification guide:
...
- [x] **4C.12** `docs/known-limitations.md` — explicit documentation of detection boundaries:
...
- [x] **4D.1** `tests/test_report_pdf.py` — PDF generates without error, page count > 0, contains expected sections (executive summary, findings table, remediation, charts)
- [x] **4D.2** `tests/test_report_html.py` — HTML renders valid markup, contains chart images, code context sections, cross-links work
- [x] **4D.3** `tests/test_executive_summary.py` — risk level calculation matches severity distribution, scan metadata accurate
- [x] **4D.4** `tests/test_remediation.py` — every PHI type has remediation guidance, suggested code replacements are syntactically valid
- [x] **4D.5** `tests/test_charts.py` — chart PNG generation succeeds, charts reflect actual findings data (severity pie, category bar, trend line)
- [x] **4D.6** `tests/test_compliance_mapping.py` — each finding maps to correct HIPAA identifier, SOC 2 control, HITRUST requirement
- [x] **4D.7** `tests/test_report_formats.py` — `--output pdf`, `--output html` produce valid files; filenames default correctly; `--report-path` respected
- [x] **4D.8** `tests/test_multi_framework.py` — `--framework soc2`, `--framework hipaa+hitrust` produce correct combined compliance sections

### Phase 4 Verification Checklist

- [x] PDF report opens correctly, contains charts, findings table, and remediation
- [x] HTML report renders in browser, charts visible, code context expandable
- [x] Executive summary risk level matches findings severity
- [x] Every finding includes remediation guidance
- [x] Charts accurately reflect findings data
- [x] `--output pdf` and `--output html` produce valid files
- [x] `--framework soc2` adds SOC 2 control mappings to findings
- [x] `--framework gdpr` annotates health data findings with Article 9 (special categories)
- [x] `--framework 42cfr2` annotates SUD-related findings with 42 CFR Part 2 notice
- [x] `--framework gina` annotates genetic identifier findings with GINA protections
- [x] `--framework bipa` annotates biometric findings with BIPA private right of action notice
- [x] PDF/HTML reports include multi-framework compliance sections
- [x] `phi-scan explain frameworks` lists all supported compliance frameworks with penalty ranges
- [x] `phi-scan explain deidentification` documents Safe Harbor vs Expert Determination gap
- [x] `docs/compliance-frameworks.md` complete with citations, controls, and penalties
- [x] `docs/de-identification.md` complete with known gaps (PDF, DICOM, Office docs)
- [x] `docs/known-limitations.md` complete and honest about detection boundaries
- [x] All documentation files complete and cross-linked
- [x] CONTRIBUTING.md merged to repo
...
- [x] **5A.1** Build `notifier.py` — email notification module
- [x] **5A.2** SMTP support — configurable host, port, TLS, from address
- [x] **5A.3** Configurable recipients from `.phi-scanner.yml` (secops, repo-owner, compliance)
- [x] **5A.4** Rich-formatted email template — findings table, remediation hints
- [x] **5A.5** Email fields: subject with repo/branch/PR#, committer info, scanner version
- [x] **5A.6** TLS required for all SMTP connections — no plaintext email
- [x] **5A.7** Attach PDF or HTML report to email when `fail_on_violation` triggers
- [x] **5A.8** Email subject format: `[PHI ALERT] {risk_level} — {findings_count} findings in {repo}/{branch}`

### 5B — Webhook Notifications

- [x] **5B.1** httpx async webhook client — POST findings JSON to configured URL
- [x] **5B.2** Slack webhook support — formatted message with findings summary
- [x] **5B.3** Microsoft Teams webhook support
- [x] **5B.4** Generic webhook — POST JSON to any URL (PagerDuty, custom)
- [x] **5B.5** Retry logic — configurable retry count on webhook failure

### 5C — Audit Log Hardening

- [x] **5C.1** Finalize SQLite schema — `scan_events` table with full findings JSON
- [x] **5C.2** Enforce immutability — application-level guards against UPDATE/DELETE
- [x] **5C.3** Audit log entry schema matches PDF spec:
...
- [x] **5C.4** Log rotation — retention policy respecting `AUDIT_RETENTION_DAYS` (2192 days / 6 years)
- [x] **5C.5** `phi-scan history` command — query by date range, repo, violation-only filter
- [x] **5C.6** `phi-scan report` command — display last scan with Rich formatting
- [x] **5C.7** Trend analysis queries — supply data for trend charts (findings over time, by repo, by category)
- [x] **5C.8** Hash chain tamper evidence — each `scan_events` row stores an HMAC-SHA256
...
- [x] **5C.9** Audit log encryption at rest — encrypt `audit.db` using SQLCipher or
...
- [x] **5D.1** Test email notification sends on PHI detection (mock SMTP)
- [x] **5D.2** Test webhook POST with mock server
- [x] **5D.3** Test audit log insert and query roundtrip
- [x] **5D.4** Test retention policy does not delete within 6-year window
- [x] **5D.5** Test immutability — verify UPDATE/DELETE raise errors

### Phase 5 Verification Checklist

- [x] Email sent on PHI detection with correct subject and body
- [x] Webhook fires to Slack/Teams/generic URL
- [x] Audit log entries are immutable (no UPDATE/DELETE)
- [x] `phi-scan history --last 30d` returns correct results
- [x] `phi-scan report` displays last scan with Rich formatting
- [x] Retention policy is HIPAA-compliant (6 years minimum)
- [x] `phi-scan history --verify` recomputes chain hash and reports PASS or FAIL
- [x] Tampered row detected: manually modifying a row causes `--verify` to fail with clear message
- [x] Audit log encrypted at rest; unencrypted file not readable without key
...
- [x] **6A.1** GitHub Actions workflow — `.github/workflows/phi-scan.yml`
- [x] **6A.2** GitLab CI job template — `.gitlab-ci.yml` snippet:
...
- [x] **6A.3** Jenkins pipeline stage — `Jenkinsfile` snippet:
...
- [x] **6A.4** Azure DevOps YAML step — `azure-pipelines.yml`:
...
- [x] **6A.5** CircleCI job — `.circleci/config.yml`:
...
- [x] **6A.6** Bitbucket Pipelines step — `bitbucket-pipelines.yml`:
...
- [x] **6A.7** AWS CodeBuild — `buildspec.yml`:
...
- [x] **6B.1** Write `docker/Dockerfile` — Alpine-based, phi-scan pre-installed
- [x] **6B.2** Write `docker/docker-compose.yml` — local testing setup
...
- [x] **6C.1** `gh` CLI integration — post PR comment with findings table
...
- [x] **6C.6** GitLab MR note (comment) posting via GitLab API:
...
- [x] **6C.7** GitLab Code Quality report support:
...
- [x] **6C.8** GitLab SAST report support:
...
- [x] **6C.9** GitLab external status check integration:
...
- [x] **6C.16** Azure DevOps PR thread comment via REST API:
...
- [x] **6C.19** CircleCI PR comment posting:
...
- [x] **6C.22** Bitbucket PR comment posting via REST API:
...
- [x] **6C.23** Bitbucket Code Insights integration:
...
- [x] **6C.24** Bitbucket build status:
...
- [x] **6C.25** AWS CodeBuild PR comment posting:
...
- [x] **6D.1** End-to-end test: GitHub Actions workflow scans repo with synthetic PHI
