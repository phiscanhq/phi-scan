...
- [x] **7A.9** BYOAK — read API key from `ANTHROPIC_API_KEY` env var; clear error if AI enabled but no key found
...
model name. No third-party routing library — lean per-provider adapters only.

- [x] **7D.1** Add `ai-anthropic`, `ai-openai`, `ai-google` optional extras to `pyproject.toml`; keep `ai` as meta-extra defaulting to `ai-anthropic`
- [x] **7D.2** Add `OPENAI_API_KEY_ENV_VAR`, `GOOGLE_API_KEY_ENV_VAR`, per-provider cost rate constants, `AI_DEFAULT_MODEL`, `AIProviderName(StrEnum)`, and model-prefix detection constants to `constants.py`
- [x] **7D.3** Define `AIProvider` protocol in `ai_review.py` with `call_review_api(prompt, model) -> tuple[str, int, int]`
- [x] **7D.4** Implement `_AnthropicProvider`, `_OpenAIProvider`, `_GoogleProvider` adapters — each within 30 lines, lazy SDK import, per-instance scoped API key
- [x] **7D.5** Add `_detect_provider_name(model)`, `_resolve_key_for_provider(provider_name)`, and `_build_provider_adapter(provider_name, api_key)` factory
- [x] **7D.6** Replace `AIReviewConfig.api_key` with `AIReviewConfig.model` (defaults to `AI_DEFAULT_MODEL`); update `resolve_api_key(model: str)` signature
- [x] **7D.7** Update `config.py`: add `enable_ai_review` key; emit `DeprecationWarning` for `enable_claude_review`; raise `ConfigurationError` if API key set in YAML; add `model` field
- [x] **7D.8** Update `exceptions.py`, `help_text.py` — remove Anthropic-specific language from user-facing text
- [x] **7D.9** Update `test_ai_review.py`: provider-agnostic mocking, provider-detection tests, PHI safety sentinels for all three providers
- [x] **7D.10** Security fix: replace global `genai.configure(api_key=...)` with `genai.Client(api_key=...)` scoped to each call — eliminates cross-tenant key exposure in concurrent environments
- [x] **7D.11** Extract `_ANTHROPIC_API_ERROR_TEMPLATE`, `_OPENAI_API_ERROR_TEMPLATE`, `_GOOGLE_API_ERROR_TEMPLATE`, and `AI_ZERO_TOKEN_COUNT` constants — eliminate all magic literals in provider adapters

### Phase 7 Verification Checklist

- [x] `[REDACTED]` appears in every AI API request — zero raw PHI leaks
- [x] False positive rate reduced measurably with AI enabled vs disabled
- [x] AI API failure falls back to local score — scanner does not crash
- [x] `ai.enable_ai_review: false` (default) produces zero API calls
- [x] Missing API key with AI enabled raises `AIConfigurationError` with clear message
...
- [x] Anthropic (`claude-*`), OpenAI (`gpt-*`, `o1`/`o3`/`o4`), and Google (`gemini-*`) model names each route to the correct provider adapter

### 7E — Security Hardening

**Goal:** Close four confirmed security gaps identified in the full repository
technical review: ZIP decompression bomb protection, webhook SSRF, unescaped HTML
in email notifications, and a factually incorrect security claim in README/docs.
All are small, targeted fixes with no behavior change for well-formed inputs.

**Dependencies:** Phase 7D complete.

- [x] **7E.1** `scanner.py` — ZIP decompression bomb guard: check `ZipInfo.file_size`
  before calling `archive.read(member_name)`; enforce per-member uncompressed size
  limit (constant `ARCHIVE_MAX_MEMBER_UNCOMPRESSED_BYTES`); log WARNING and skip
  members that exceed the limit or have a suspicious compression ratio
- [x] **7E.2** `notifier.py` — Webhook SSRF protection: validate `webhook_url` scheme
  is `https` before posting; block RFC1918 / link-local / metadata IP ranges
  (169.254.x.x, 100.64.x.x, 10.x, 172.16–31.x, 192.168.x); raise
  `NotificationError` with clear message on invalid URL; add opt-out config flag
  `is_private_webhook_url_allowed: false`
- [x] **7E.3** `notifier.py` — HTML email escaping: wrap all dynamic fields
  (`file_path`, `repo`, `branch`, `category`, `severity`, `risk_level`) with
  `html.escape()` before `.format()` interpolation into HTML templates; add test
  asserting `<script>` in branch name is escaped in output
- [x] **7E.4** `README.md` + `docs/` — AI review caveat: qualify the "no PHI ever
  leaves your infrastructure" and "no data sent to any external service — ever"
  claims with "unless AI review is explicitly enabled" in the tagline, quick-start,
  and Why PhiScan sections
- [x] **7E.5** `docs/security.md` — document ZIP decompression bomb protection:
  explain the per-member uncompressed size limit, the compression ratio guard,
  and what happens when a member is skipped (WARNING log, scan continues)
- [x] **7E.6** `docs/known-limitations.md` — update the ZIP/JAR/WAR section to
  reflect the new decompression bomb guard and document the per-member size limit
  constant so users know the boundary
- [x] **7E.7** `docs/configuration.md` — document the new `is_private_webhook_url_allowed`
  config flag: default (`false`), what it blocks (RFC1918, link-local, metadata IPs),
  and when to enable it (self-hosted GitLab, on-premise Jenkins behind a private LB)
- [x] **7E.8** `docs/security.md` — add a Webhook Security section documenting the
  SSRF protection model: scheme allowlist, IP block list, and the opt-out flag

### 7E Verification Checklist

- [x] A ZIP member with uncompressed size exceeding limit is skipped with a WARNING log
- [x] A compression ratio > safety threshold triggers the per-member guard
- [x] `webhook_url = "http://..."` raises `NotificationError` (scheme rejected)
- [x] `webhook_url` pointing to 169.254.169.254 raises `NotificationError` (metadata blocked)
- [x] `<script>` in `branch` name is HTML-escaped in the email body
- [x] README tagline and Why PhiScan bullet both carry the AI opt-in qualifier

---

### 7F — Architecture: Maintainability Refactors

**Goal:** Reduce velocity drag in the three largest files identified in the technical
review. No behavior changes — pure structural refactors with test coverage preserved.
Each sub-task is independently shippable as its own PR.

**Dependencies:** 7E complete (security clean first).

#### 7F.1 — Split `output.py` (2339 LOC) into focused modules

The file mixes three distinct concerns. Split into a package:

```
phi_scan/output/
    __init__.py          re-exports all public symbols (backwards-compatible)
    serializers.py       format_json, format_csv, format_sarif, format_junit,
                         format_codequality, format_gitlab_sast + their builders
    console.py           display_*, build_*_panel, banner, progress, file tree
    watch.py             WatchEvent, build_watch_layout, watch builders
