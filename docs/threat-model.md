...
| A-2 | Zip slip (path traversal via `..` in archive member names). | **P1** | PhiScan never writes archive members to disk. Each member is read into memory via `ZipFile.read` and scanned in place. `ZipFile.extract` / `extractall` are explicitly not called anywhere in the codebase. | `tests/test_module_structure.py::test_phi_scan_package_contains_no_archive_extract_calls` — AST walk over every Python source file under `phi_scan/` that fails on any `.extract(...)` or `.extractall(...)` method call. Also `test_scanner.py` archive-handling tests confirm reads happen via `ZipFile.read`. | None known. The mitigation is structural — no write path exists, and the structural test is a CI regression gate. |
...
| F-3 | Fixer executes attacker-controlled code (e.g. through a subprocess or eval). | **P0** | The fixer is pure Python text replacement. No subprocess, shell, `eval`, `exec`, `compile`, `__import__`, `os.system`, `os.popen`, `os.exec*`, or `os.spawn*` primitive occurs anywhere in `phi_scan/fixer.py`. | `tests/test_fixer.py::test_fixer_module_contains_no_dynamic_code_execution` — parses `phi_scan/fixer.py` with `ast` and fails on any banned import, banned call name, or banned attribute chain. Any future change introducing code-execution primitives fails this regression gate and requires an explicit security review. | None known. |
...
- **I-3 ReDoS stress coverage**: row I-3 above notes there is no
  dedicated ReDoS stress test for detector regex patterns today. A
  future hardening PR should add a bounded worst-case-input benchmark
  gate that compiles every pattern and times it against a pathological
  input, failing above a fixed threshold. Candidate for a future
  `security/*` PR; P2, not blocking v1.0.
